        ...
        void gen_expr(const NodeExpr& expr) {
            struct ExprVisitor {
                ...
                void operator()(const NodeExprInt& expr_int) {
                    gen->m_output << "    mov rax, " << expr_int._int.value.value() << "\n";
                    gen->push("rax");
                }
        ...
        void gen_stmt(const NodeStmt& stmt) {
            struct StmtVisitor {
                ...
                void operator()(const NodeStmtSet& stmt_set) const {
                    if (gen->m_vars.contains(stmt_set.ident.value.value())) {
                        std::cerr << "Indentifer already used: " << stmt_set.ident.value.value() << std::endl;
                        exit(EXIT_FAILURE);
                    }
                    gen->m_vars.insert({stmt_set.ident.value.value(), Var {.stack_loc = gen->m_stack_size}});
